...
class SessionType(enum.IntEnum):
    AIR_SIM = 0x3C  # airplanes, simulator battles (60)
    MARINE_BATTLE = 0x1a  # naval battles (26)
    RANDOM_BATTLE = 0x20 # tank battles (32)
    CUSTOM_BATTLE = 0x40  # test drive ? (64)
    USER_MISSION = 0x01  # user missions (1)
...
Header = ct.Struct(
    ...
    'unk_35' / ct.Bytes(35),
    'session_type' / ct.Byte,  # I am only interested in RANDOM_BATTLE for tanks
    'unk_3' / ct.Bytes(3),
...
def FatBlockStream(sz: t.Union[int, callable, None] = None) -> ct.Construct:
    ...
    return ct.RestreamData(con, blkbin.Fat)
...
WRPLCliFile = ct.Struct(
    ...
    #'rez_offset' / ct.Tell,
    #ct.Seek(this.header.rez_offset, 0),  # Seek to the rez_offset from the beginning of the file
    'rez' / blkbin.Fat,

    # Cannot find zlib headers
