...
def serialize(root: Section, out_format: Format, is_sorted: bool = False, check_cycle: bool = False) -> str:
    mapper = {
    ...
    if root:
        ...
        if isinstance(root, DictSection):
            return json.dumps(mapper.map(root), cls=NoIndentEncoder, ensure_ascii=False, indent=2, separators=(',', ': '),
                              sort_keys=is_sorted)
